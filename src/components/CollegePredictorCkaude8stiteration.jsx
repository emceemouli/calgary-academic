...
const CollegePredictor = () => {
  ...
  const classifySchool = (
    ...
    const gpaAboveRange = inputGpa - gpaRange.max;
    const gpaBelowRange = gpaRange.min - inputGpa;
    const satAboveRange = inputSat - satRange.max;
    const satBelowRange = satRange.min - inputSat;

    const isWellAboveRange = () => {
      return (
        gpaAboveRange > 0.2 ||
        (gpaAboveRange > 0 && satAboveRange > 50) ||
        satAboveRange > 100
      );
    };

    const isWellBelowRange = () => {
      return (
        gpaBelowRange > 0.3 ||
        (gpaBelowRange > 0 && satBelowRange > 100) ||
        satBelowRange > 150
      );
    };
    ...
    const isInRange = () => {
      return (
        ...
        inputGpa <= (gpaRange.max + 0.2) &&
        inputSat >= (satRange.min - 75) &&
        inputSat <= (satRange.max + 75)
      );
    };
    ...
    if (
      (
        ...
        (
          ...
          (
            (acceptanceRate > 0.5 && rank > 50) ||
            (acceptanceRate > 0.7) ||
            (scoreProfile.tier >= 4 && rank > 100)
          )
        )
      )
    ) {
    ...
    if (
      (isInRange() || 
      (Math.abs(gpaAboveRange) <= 0.2 && Math.abs(satAboveRange) <= 75)) &&
      (
        (acceptanceRate >= 0.2 && acceptanceRate <= 0.6) ||
        (scoreProfile.tier >= 3 && acceptanceRate >= 0.15) ||
        (rank > 30 && rank <= 100 && isInRange())
      )
    ) {
      return 'Target';
    }
    ...
    if (
      (rank <= 10) || // Top schools are always reach unless exceptional stats
      (rank <= 30 && !isWellAboveRange() && scoreProfile.tier < 5) ||
      (rank <= 50 && gpaBelowRange > 0 && !isWellBelowRange()) ||
      (acceptanceRate < 0.15 && !isWellBelowRange()) ||
      (gpaBelowRange > 0 && satBelowRange > 0 && acceptanceRate < 0.3)
    ) {
      return 'Reach';
    }

    // If no clear classification but stats are close, default to Target
    if (isInRange() && rank <= 150) {
      return 'Target';
    }
  ...
  const handlePrediction = async () => {
    ...
    try {
      ...
      // First pass: classify all schools
      ...
      // Second pass: ensure balanced distribution
      ['Reach', 'Target', 'Safety'].forEach(category => {
        const schoolsInCategory = classifications
          .filter(c => c.classification === category)
          .sort((a, b) => {
            if (category === 'Safety') {
              // For safety, prioritize higher acceptance rates
              return b.acceptanceRate - a.acceptanceRate;
            } else if (category === 'Reach') {
              // For reach, prioritize better ranks
              return a.rank - b.rank;
            }
            // For target, balance rank and acceptance rate
            return a.rank - b.rank;
          })
          .slice(0, 5);
      ...
      // If any category is empty, try to fill it from nearby categories
      if (processedResults.Target.length === 0) {
        const potentialTargets = classifications
          .filter(c => c.classification === 'Safety' && c.rank <= 100)
          .slice(0, 2);
        processedResults.Target.push(...potentialTargets.map(s => s.university));
      }
