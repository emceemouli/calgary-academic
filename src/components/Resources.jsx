...
import { BookOpen, Brain, Download, Calculator, GraduationCap } from 'lucide-react';
...
const Resources = () => {
  ...
  useEffect(() => {
    ...
    if (metaDescription) {
      metaDescription.setAttribute('content', 'Access free Digital SAT practice materials, study guides, and sample questions. Expert-curated content for comprehensive SAT preparation in Calgary.');
    }
  }, []);
  ...
  const educationalContent = {
    math: [{
      ...
      previewContent: ["Sample Problem: Linear Equations", "A company's profit P (in dollars) is given by P = 200x - 1500, where x is the number of units sold.", "Question: How many units must be sold to break even (P = 0)?", "Solution:", "1. Set P = 0", "2. Solve: 0 = 200x - 1500", "3. Result: x = 7.5, therefore 8 units needed"],
    ...
    verbal: [{
      ...
      previewContent: ["Key Strategy: Active Reading", "1. Preview passage structure", "2. Read first and last paragraphs", "3. Identify topic sentences"],
    ...
    practice: [{
      ...
      previewContent: ["Test Overview:", "• 54 Math questions", "• 27 Reading questions", "• 27 Writing questions"],
  ...
  return (
    <div className="min-h-screen bg-gray-50 w-full overflow-x-hidden">
      <div className="relative h-[250px] mt-16 w-full">
        ...
        <div className="relative container mx-auto px-4 h-full flex flex-col justify-center max-w-[1200px]">
      ...
      <div className="container mx-auto px-4 py-8 max-w-[1200px]">
        <div className="flex flex-wrap gap-3 mb-8 pb-4 border-b border-gray-200">
          {navigationItems.map(item => (
            <Button key={item.id} variant={activeSection === item.id ? 'default' : 'outline'} onClick={() => setActiveSection(item.id)} className="flex items-center gap-2 px-4 py-2 text-base">
              ...
              <CardContent className="pt-6">
                ...
                <div className="bg-gray-50 p-6 rounded-lg text-gray-800">
                  {resource.previewContent.map((line, i) => (
                    <p key={i} className="mb-3 last:mb-0 text-base sm:text-lg leading-relaxed">{line}</p>
                  ))}
                </div>
                <div className="mt-6 flex gap-4">
                  <Button className="flex-1 sm:flex-none bg-blue-600 hover:bg-blue-700 text-white">Start Learning</Button>
