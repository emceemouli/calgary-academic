...
jsonData.forEach((row) => {
    ...
    // Acceptance rate is now directly in decimals
    ...
    if (gpaRange.min === 0 && gpaRange.max === 0) {
        console.warn(`Invalid GPA range for ${row.University}`);
        return;
    }
    if (satRange.min === 0 && satRange.max === 0) {
        console.warn(`Invalid SAT range for ${row.University}`);
        return;
    }
