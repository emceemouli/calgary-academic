...
/**
 * Elite ChanceMe — Single School Only
 * - User chancing for ONE school only
 * - AI explanation for that ONE school only (optional)
 * - Searchable dropdown (type-to-filter) for switching schools
 * - Uses CSV at: /excel/cleaned_collegerankings.csv
 *
 * CSV headers expected (matches your sample):
 * Rank, University, Type, GPA_Range, SAT_Range, ACT_Range,
 * Early_Decision, Early_Action, Application_Fee, Acceptance_Rate,
 * Average_Aid, Need_Blind, Merit_Aid, Special_Programs, Notable_Strengths
 */
...
export default function EliteChanceMe() {
  ...
  // AI explanation — ONLY for the selected school
  ...
  const computed = useMemo(() => {
    ...
    // Similar schools by closeness in probability
    const similar = db
  ...
  async function generateAIExplanation() {
    ...
    try {
      const prompt = `
You are a former elite admissions officer. Explain ONE school's admissions chance estimate.
Return concise, specific, non-generic guidance.
...
INSTRUCTIONS:
Write:
1) 4-6 bullets: specific reasons tied to GPA/test ranges and rigor/EC/fit
2) 3 improvements that would most increase odds for THIS school
3) 2 narrative spike ideas aligned to school strengths
Avoid mentioning other universities. Avoid generic filler.
      `.trim();
      ...
      if (!text) {
        setAiError("AI returned no text. Check API key and model access.");
      } else {
        setAiText(text);
      }
    } catch (e) {
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="mx-auto max-w-7xl px-6 py-10">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-slate-600 font-medium mt-1">
              One-school probability + reasons. Optional AI explanation for this school only.
            </p>
          </div>
        </div>
        ...
        <div className="mt-8 grid lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-5 rounded-[2.5rem] border border-slate-100 bg-white p-8 shadow-sm">
            ...
            <div className="mt-6">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`px-4 py-2 rounded-full font-black text-xs tracking-widest ${
                    profile.testType === "sat" ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-700"
                  }`}
                >
                  SAT
                </button>
                <button
                  ...
                  className={`px-4 py-2 rounded-full font-black text-xs tracking-widest ${
                    profile.testType === "act" ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-700"
                  }`}
                >
                  ACT
                </button>
            ...
            <button
              ...
              title="Generate explanation for this school only"
            >
            ...
            <div className="mt-4 text-xs text-slate-500 leading-relaxed">
              ...
              Works without AI (probability + reasons). AI requires <span className="font-black">VITE_GOOGLE_AI_KEY</span>.
            </div>
          </div>
          ...
          <div className="lg:col-span-7 space-y-8">
            ...
            {(aiLoading || aiText) && (
              <div className="rounded-[2.5rem] border border-slate-100 bg-white p-8 shadow-sm">
                <div className="font-black text-xl">AI explanation (this school only)</div>
                <div className="text-slate-600 font-medium mt-1">Generated specifically for {school.name}.</div>
                <div className="mt-5 whitespace-pre-wrap text-slate-700 font-medium leading-relaxed">
                ...
                {computed.similar.map(({ s, r }) => (
                  ...
                  >
                    ...
                    <div className="text-sm text-slate-600 font-semibold">
                      {formatPct(r.p)} • {r.label} • {s.type} #{s.rank}
                    </div>
                  </button>
                ))}
              </div>
        ...
        <div className="mt-10 text-xs text-slate-500">
          CSV source: <span className="font-black">{CSV_PATH}</span>. This page intentionally chances for one school only.
        </div>
      </div>
    </div>
  );
