...
const CollegePredictor = () => {
  ...
  const handlePrediction = async () => {
    ...
    try {
      ...
      sortedSchools.forEach((row) => {
        ...
        console.log(`\nAnalyzing ${row.University}:`);
        console.log(`GPA Range: ${gpaRange.min}-${gpaRange.max}, Input GPA: ${inputGpa}`);
        console.log(`SAT Range: ${satRange.min}-${satRange.max}, Input SAT: ${inputSat}`);
        console.log(`Rank: ${rank}, Acceptance Rate: ${acceptanceRate}`);
  ...
  const classifySchool = (
    ...
    if (
      ...
      console.log(`Classified as Safety: Scores above range = ${isWellAboveRange()}, AcceptanceRate = ${acceptanceRate}`);
      return 'Safety';
    }

    // Reach School Classification
    if (
      (rank <= 10 && !isWellBelowRange()) || 
      (rank <= 30 && gpaBelowRange > 0 && gpaBelowRange < 0.2) ||
      (rank <= 50 && acceptanceRate < 0.15 && !isWellBelowRange()) ||
      (rank > 50 && gpaBelowRange > 0 && satBelowRange > 0 && acceptanceRate < 0.2)
    ) {
      console.log(`Classified as Reach: Below range = ${isWellBelowRange()}, Rank = ${rank}`);
      return 'Reach';
    }

    // Target School Classification
    if (
      Math.abs(gpaAboveRange) <= 0.3 &&
      Math.abs(satAboveRange) <= 100 &&
      Math.abs(gpaBelowRange) <= 0.2 &&
      Math.abs(satBelowRange) <= 75 &&
      acceptanceRate >= 0.15 &&
      acceptanceRate <= 0.5
    ) {
      console.log(`Classified as Target: Within range margins`);
      return 'Target';
    }
