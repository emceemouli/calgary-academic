...
const CollegePredictor = () => {
  ...
  const classifySchool = (
    ...
    if (
      (inputGpa >= gpaRange.min + 0.2 && inputSat >= satRange.min + 50) ||
      (acceptanceRate > 0.7) ||
      (inputGpa >= gpaRange.min && inputSat >= satRange.min && acceptanceRate > 0.5)
    ) {
      return 'Safety';
    }
    ...
    if (
      ...
      (
        (rank > 100) ||
        (rank > 50 && inputGpa >= gpaRange.min && inputSat >= satRange.min)
      )
    ) {
      return 'Target';
    }
    ...
    if (
      !isOutOfRange() &&
      (
        (gpaGap < 0.3 && satGap < 100) ||
        (acceptanceRate > 0.2 && gpaGap < 0.2) ||
        (rank > 30 && rank <= 100 && gpaGap < 0.4 && satGap < 150)
      )
    ) {
      return 'Reach';
    }
  ...
  const handlePrediction = async () => {
    ...
    try {
      ...
      if (processedResults.Target.length === 0 && processedResults.Safety.length > 2) {
        const promotedSafety = classifications
          .filter(c => c.classification === 'Safety' && c.rank <= 150)
          .sort((a, b) => a.rank - b.rank)
          .slice(0, 2);
        processedResults.Target = promotedSafety.map(s => s.university);
        processedResults.Safety = processedResults.Safety
          .filter(school => !promotedSafety.find(s => s.university === school));
      }

      setResults(processedResults);

      console.log("Input Scores - GPA:", inputGpa, "SAT:", inputSat);
