  ...
  const handlePrediction = async () => {
    ...
    try {
      ...
      jsonData.forEach((row) => {
        ...
        let acceptanceRate = row.Acceptance_Rate;
        if (typeof acceptanceRate === "string") {
          acceptanceRate = parseFloat(acceptanceRate.replace("%", "").trim()) / 100;
        } else if (typeof acceptanceRate === "number") {
        ...
        console.log("GPA Range:", gpaRange, "SAT Range:", satRange, "Acceptance Rate:", acceptanceRate);
        ...
        if (
          ...
          (acceptanceRate > 0 && acceptanceRate < 0.2)
        ) {
