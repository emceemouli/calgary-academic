...
const CollegePredictor = () => {
    ...
    try {
      ...
      jsonData.forEach((row) => {
        ...
        debugLogs.push(`
          ...
          Raw Acceptance Rate: ${row.Acceptance_Rate}
          Parsed Acceptance Rate: ${acceptanceRate}
        ...
        if (isReachSchool(inputGpa, inputSat, gpaRange, satRange, acceptanceRate)) {
          if (processedResults.Reach.length < 5) {
            processedResults.Reach.push(row.University);
            debugLogs.push(`Classified as Reach: ${row.University}`);
          }
          ...
          if (processedResults.Target.length < 5) {
            ...
            debugLogs.push(`Classified as Target: ${row.University}`);
          }
          ...
          if (processedResults.Safety.length < 5) {
            ...
            debugLogs.push(`Classified as Safety: ${row.University}`);
          }
        }
      });
  ...
  const parseAcceptanceRate = (rate) => {
    if (typeof rate === "string") {
      // Handle percentage strings (e.g., "4.0%")
      return parseFloat(rate.replace("%", "")) / 100;
    } else if (typeof rate === "number") {
      // Handle decimal numbers (e.g., 0.04)
      return rate < 1 ? rate : rate / 100;
    }
    return 0;
  };

  const isReachSchool = (inputGpa, inputSat, gpaRange, satRange, acceptanceRate) => {
    return (
    ...
    <div className="container mx-auto py-16 px-4 bg-white shadow rounded-md">
      ...
      {results && (
        <>
          ...
          <div className="mt-8 max-w-2xl mx-auto bg-gray-100 p-4 rounded-md">
